...
def book_share(request, service_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            subject = '{} ({}) wants to buy your book "{}"'.format(cd['name'], cd['Your_email'], service.name)
            print(subject)
            message = 'Read "{}" at {}\n\n{}\'s message: \n"{}'.format(service.name, service_url, cd['name'],cd['message'])
            print(message)
            send_mail(subject, message, 'hello@gmail.com', [cd['Seller_email']])
            sent = True
    else:
        form = EmailPostForm()
